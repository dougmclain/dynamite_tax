...
class AssociationForm(forms.ModelForm):
    state = forms.ChoiceField(choices=US_STATE_CHOICES, widget=forms.Select(attrs={'class': 'form-select tom-select'}))
    records_state = forms.ChoiceField(choices=US_STATE_CHOICES, required=False, widget=forms.Select(attrs={'class': 'form-select tom-select'}))

    class Meta:
        model = Association
        fields = [
            'association_name', 'mailing_address', 'room_suite', 'care_of', 'city', 'state', 'zipcode',
            'filing_state', 'state_tax_id', 'naics_code', 'records_city', 'records_state', 'records_zip',
        ...
        widgets = {
            ...
            'filing_state': forms.Select(attrs={'class': 'form-select tom-select'}),
        }
